# MoneyPrinterTurbo - 자동 숏폼 영상 생성 + TikTok 업로드 파이프라인
...
| [Playwright](https://playwright.dev/) | TikTok 자동 업로드 | 무료 | X | 가입 불필요 |
...
venv 자동 감지 → 랜덤 주제 선택 → 영상 생성 → TikTok 업로드 → 파일 정리 → 자동 종료.
...
A. TikTok 정책상 과도한 업로드는 제한될 수 있습니다. 하루 1~3개 정도가 안전합니다.
